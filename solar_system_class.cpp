...
void Universe::generate() {
    ...
    while (window_.isOpen()) {
    	std::vector<Body*>::iterator move_itr = planets_.end();
        while (window_.pollEvent(event_)) {
            if (event_.type == sf::Event::Closed)
                window_.close();
            if (event_.key.code == sf::Keyboard::Escape)
                window_.close();
            (*move_itr) -> planetEvent(event_);
...
void Body::planetEvent(sf::Event & event) {
 ...
 /*
  if(location_.y > 1000)
  {
    this -> setLocation(sf::Vector2f(location_.x, 0));
  }
  if(location_.y < 0)
  {
    this -> setLocation(sf::Vector2f(location_.x, 1000));
  }
  if(location_.x > 1000)
  {
    this -> setLocation(sf::Vector2f(0, location_.y));
  }
  if(location_.x < 0)
  {
    this -> setLocation(sf::Vector2f(1000, location_.y));
  }
  */
}
